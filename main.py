...
bot = AsyncTeleBot(token=settings.TOKEN_BOT)
...
app = FastAPI(title='Telegram Bot Api', description='ربات دستیار برنامه نویس', version='1.0.0', lifespan=lifespan)
...
async def webhook_handler(request: Request):
    try:
        ...
        if update_data:
            await bot.process_new_updates([update_data])
        return {"status": "ok"}
...
async def admin_message_handler(m):
    chatId = m.chat.id
    if not meessage_checkAdmin(chatId):
        await bot.send_message(chatId, "⚠️ شما دسترسی ادمین ندارید.")
        return
    ...
    if text == '/AllUser':
        ...
        await bot.send_message(chatId, message_text, parse_mode='HTML')
...
async def message_All_Button(m):
    chat_id = m.chat.id
    with get_db() as db:
        ...
        if m.text == '/unblock':
            if not check_block_user(chat_id):
                await bot.send_message(chat_id,'شما بلاک نشده‌اید بنابراین نمیتوانید از این دستور استفاده کنید🙂')
                return
            if not isRequestsblock(chat_id):
                await bot.send_message(chat_id,'شما قبلا درخواست داده‌اید، لطفا منتظر نتیجه بمانید!!')
                return
            ...
            await bot.send_message(chat_id,'درخواست آنبلاک برای ادمین ارسال شد، لطفا منتظر پاسخ باشید!!😒')
        elif m.text == '/information':
            await bot.send_message(chat_id, text=text_information, parse_mode='HTML')
        elif m.text == '/about':
            await bot.send_message(chat_id, text=text_ReadMe, parse_mode='HTML')

# ---------------- ADMIN UNBLOCK HANDLER ----------------
@bot.message_handler(regexp=r"^/unblock:\d{1,15}$")
async def message_All_Admin(m):
    chatId = m.chat.id
    ...
    await bot.send_message(chatId_user,'ادمین شمارا از بلاک درآورد، تبریک می‌گویم 🙂‍')
    await next_unblock_button(chatId_user)
    await bot.send_message(settings.CHAT_ID,'کاربر با موفقیت آنبلاک شد!')
    all_button_for_Admin()

# ---------------- MESSAGE HANDLER ----------------
@bot.message_handler(func=lambda m: True)
async def control_message_for_me(message):
    chatId = message.chat.id
    ...
    if check_block_user(chatId):
        await bot.send_message(chatId, "⛔ شما بلاک شدید.")
        return
    ...
    if ai_toxic.get("toxic") or ai_toxic.get("score",0)>=0.65:
        ...
        await bot.send_message(chatId,f"⚠️ پیام نامناسب شناسایی شد. این {warnings}‌مین اخطار شماست.")
        if warnings>=5:
            updateUser(chat_id=chatId,data=UserUpdate(isBlock=True))
            await unblock_button(chatId)
            await bot.send_message(chatId,"⛔ شما بلاک شدید.")
        return
    ...
    if intent in ["project","contact"] and confidence>=0.55:
        ...
        if msg_to_user:
            await bot.send_message(chatId, msg_to_user)
        if status=="complete" and msg_to_admin:
            await bot.send_message(settings.CHAT_ID, msg_to_admin)
        return
    ...
    if intent in ["greeting","spam_or_joke"] and confidence>=0.6:
        ...
        await bot.send_message(chatId,response or "سلام! خوش اومدی 🙂")
        return

    # پاسخ تصادفی
    await bot.send_message(chatId, random.choice(random_text))
